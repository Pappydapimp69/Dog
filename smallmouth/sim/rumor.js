...
// Distortion accrues from three things, not one.
//
// The first build charged distortion PER HOP only, at 0.18, needing 3 hops to
// clear the 0.5 threshold. But chains top out at 2 hops, because news has a
// shelf life — so mutation was unreachable code, and lowering the threshold
// would have been the wrong fix. brain (dog#E91): when a quantity is bounded by
// the STRUCTURE of the thing (here: how far a story travels in a week), tuning
// the coefficient widens nothing; change what the quantity is a function of.
//
// What actually degrades a story is the retelling, how long the teller sat on
// it before retelling (you recall it wrong), and who is doing the telling (a
// chatty mouth sharpens a story to keep it worth hearing). Two of those three
// are available at hop 1, so a week-long chain can now degrade honestly.
export const DISTORT_PER_HOP = 0.15;
export const STALE_PER_HOUR = 0.02;    // memory of the claim erodes while held
export const EMBELLISH = 0.1;           // scaled by the teller's gossip trait
...
export function makeRumor({ claim, origin, at }) {
  return {
    ...
    distortion: 0,
    hops: 0,
    heldSince: at,          // when THIS holder got it — drives staleness on retell
 ...
 *
 * @param teller  the agent doing the retelling — their gossip trait embellishes.
 *                Optional so the token stays testable in isolation.
 */
export function retell(token, to, at, roll, teller = null) {
  const held = Math.max(0, at - token.heldSince);
  const gossip = teller ? teller.traits.gossip : 0.5;
  const added = DISTORT_PER_HOP + held * STALE_PER_HOUR + gossip * EMBELLISH;
  const next = {
    ...token,
    claim: { ...token.claim },
    distortion: token.distortion + added,
    hops: token.hops + 1,
    heldSince: at,
...
export function serializeRumor(t) {
  return {
    id: t.id, origin: t.origin, bornAt: t.bornAt, heldSince: t.heldSince, hops: t.hops,
